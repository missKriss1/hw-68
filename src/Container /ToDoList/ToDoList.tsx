...
import { fetchTodoList, fetchToDoListDelete, fetchToDoListPost, fetchTodoListPutByChek, } from './toDoListSlice';
import Spinner from '../../UI/Spinner/Spinner.tsx';
import * as React from 'react';

const ToDoList = () => {
  const dispatch:AppDispatch = useDispatch();
  const { todolist, loading, error } = useSelector((state: RootState) => state.toDoList);
            ...
            {todolist && todolist.length > 0 ? (
                todolist.map((todo) => (
                  <div key={todo.id} className="row">
                    ...
                    <div className="col-2">
                      <button onClick={() => deleteToDoList(todo.id)} className="btn btn-close"></button>
                    </div>
                    <hr/>
                  </div>
                ))
